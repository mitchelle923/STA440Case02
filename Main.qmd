...
For ease of use, we depended on conjugacy to derive the parameters of the normal distribution. Since both the mean and variance of the normal distribution are unknown, we used normal-inverse gamma priors for both parameters. At this point, the issue arises that a plurality of athletes have only ever done a single competition in any given apparatus, resulting in a large amount of athletes with 0 variance and skewing the distribution. Due to the unlikely nature of athletes who do not compete regularly participating in the Olympics, we have decided to truncate the apparatus level data set to exclude athletes with less than three competition appearances for that given apparatus.
...
We ran simulations for each apparatus for each gender (women's 4 apparatuses and men's 6 apparatuses) 1000 times. Overall, the most successful American athletes were: 
...
With a total of (number of medals) across 1000 simulations of each apparatus.
...
FX_w_simul <- simulate_full_competition(combined, "FX", "w", mu_0, sig_0, k_0, v_0, 1000)
...
BB_w_simul <- simulate_full_competition(combined, "BB", "w", mu_0, sig_0, k_0, v_0, 1000)
...
VT_w_simul <- simulate_full_competition(combined, "VT", "w", mu_0, sig_0, k_0, v_0, 1000)
VT_w_simul |>
  head(10) |>
  mutate(unique_id = case_when(unique_id == "SimBIL_USA" ~ "Simone Biles: USA",
                               unique_id == "YaqZHO_CHN" ~ "Yaqin Zhou: CHN",
...
UB_w_simul <- simulate_full_competition(combined, "UB", "w", mu_0, sig_0, k_0, v_0, 1000)
```
...
FX_m_simul <- simulate_full_competition(combined, "FX", "m", mu_0, sig_0, k_0, v_0, 1000)
```
...
VT_m_simul <- simulate_full_competition(combined, "VT", "m", mu_0, sig_0, k_0, v_0, 1000)
...
PH_m_simul <- simulate_full_competition(combined, "PH", "m", mu_0, sig_0, k_0, v_0, 1000)
```
...
HB_m_simul <- simulate_full_competition(combined, "HB", "m", mu_0, sig_0, k_0, v_0, 1000)
```
...
SR_m_simul <- simulate_full_competition(combined, "SR", "m", mu_0, sig_0, k_0, v_0, 1000)
```
...
PB_m_simul <- simulate_full_competition(combined, "PB", "m", mu_0, sig_0, k_0, v_0, 1000)
