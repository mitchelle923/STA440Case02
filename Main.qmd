...
We ran simulations for each apparatus for each gender (women's 4 apparatuses and men's 6 apparatuses) 500 times. Overall, the most successful American athletes were:

# LIST HERE PLZ

Indicating that the most successful american team would likely be composed of:

# LIST HERE PLZ

With a total of (number of medals) across 500 simulations of each apparatus.
...
```{r all-sims-table, include = FALSE}
...
#### Women's USA Gymnastics Team: (event specialists)
-   Simone Biles 
-   Shilese Jones 
-   Zoe Miller 
-   Konnor Mcclain 
...
Based on our simulations, Simone Biles has the highest count of gold medals in balance beam, floor exercise, and vault. Therefore, she is expected to be a strong contender in the individual apparatus events, individual all-around events, as well as a the team all-around since only three members compete in each apparatus for that event. Due to Simone Biles placing in ninth for highest number of gold medals for uneven bars, Shilese Jones and Zoe Miller are both great additions to the team. They are both specialists in uneven bars with Shilese coming in third place and Zoe coming in fifth place for number of gold medals for that apparatus. Konnor Mcclain specializes in balance beam coming in third for highest count of gold medals. Jade Carey specializes in vault coming in third place for highest count of gold medals. This selection provides two of the strongest USA specialists by medal count for almost each apparatus besides floor exercise since Simone received a whopping count of 192 gold medals in comparison the second place Brazilian contender with 58 gold medals in the simulation. (Appendix: Image 9)

#### Men's USA Gymnastics Team: mixture (primarily all-around) 
-   Brody Malone 
-   Donnell Whittenburg 
-   Colt Walker 
-   Asher Hong 
...
In order to find the expected best all-around male athletes from the simulation, we gave rankings of total count of gold medals for each apparatus, took the average of those rankings for each athlete and picked the top five athletes with the highest average ranking (Appendix: Image 13). Also, Asher Hong is an event specialist in vault that ranks second in number of simulated gold medals for that apparatus who also ranked in the top 5 predicted best all-around male athletes.
...
*insert answer for: Are there any issues with reliability or validity of the data?*
