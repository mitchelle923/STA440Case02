...
# normal_fit = fitdist(final_women$D_Score[final_women$Apparatus == "BB"], "norm")
# summary(normal_fit)
# calculate_p_value(normal_fit$estimate[[1]], normal_fit$estimate[[2]])
# 
# # Compare this to the sample mean and standard deviation
# mean(final_women$D_Score[final_women$Apparatus == "HB"])
# sd(final_women$D_Score[final_women$Apparatus == "HB"])
```
...
```{r error = FALSE}
# dscore_analysis <- function(data, apparatus) {
#   # Filtering data for the specified apparatus
#   filtered_data <- final_women[final_women$Apparatus == apparatus,]
# 
#   # Running fitdist() on normal distribution and calculating p-value
#   normal_fit <- fitdist(filtered_data$D_Score, "norm")
#   print(summary(normal_fit))
#   p_value <- calculate_p_value(normal_fit$estimate[[1]], normal_fit$estimate[[2]])
#   print(paste("P-value:", p_value))
# 
#   # Outputting sample mean and standard deviation
#   mean_val <- mean(filtered_data$D_Score)
#   sd_val <- sd(filtered_data$D_Score)
#   print(paste("Mean:", mean_val, "Standard Deviation:", sd_val))
# 
#   # Returning all calculated and fitted values
#   return(list(normal_fit = normal_fit,
#               p_value = p_value,
#               mean_val = mean_val,
#               sd_val = sd_val,
#               normal_mu_fit = normal_mu_fit,
#               invgamma_sd_fit = invgamma_sd_fit))
# }

# Usage example:
#dscore_analysis(final_women, "BB")
...
## BELOW
```{r}
# #calculate for the mus
# calculate_fits <- function(data, apparatus, meanchoice, varchoice) {
# 
#   # Fit a normal distribution to the means
#   normal_mu_fit <- fitdist(data$meanchoice, "norm")
# 
#   # Fit an inverse gamma distribution to the standard deviations (excluding zeros)
#   invgamma_var_fit <- fitdist(data$varchoice, "invgamma")
# 
#   # Return the fitted distributions
#   return(list(normal_mu_fit = normal_mu_fit, invgamma_var_fit = invgamma_var_fit))
# }
# 
# # Usage example:
# fits <- calculate_fits(women_apparatus, "BB", mean_D, var_D)
