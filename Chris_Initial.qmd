...
quantiled_data <- laterdata |>
  ...
  mutate(quantile = ntile(-Score, 5))

#summary of number of athletes competed in each competition in each round
number_athletes <- quantiled_data |>
  group_by(Competition, Round) |>
  summarise(unique_id_count = n_distinct(unique_id))

number_athletes |>
  filter(Round %in% AA_team) |>
  ggplot(aes(x = unique_id_count)) +
    geom_histogram() +
    facet_wrap(~ Round) +
...
ggplot(number_athletes, aes(x = unique_id_count)) +
  geom_histogram() +
  labs(x = "Number of Unique Athletes Competed",
       y = "Frequency",
